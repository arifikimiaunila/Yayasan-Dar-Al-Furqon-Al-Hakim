...
export default function AddAttribute ({value1, value2}: AddAttributeProps): ReactNode{
const [openModal, setOpenModal] = useState(true);
let onCloseModal = ()=>{setOpenModal(false)};
return (
    <>
        <Button gradientDuoTone="purpleToBlue" onClick={() => setOpenModal(true)} id="add_attribute" data-modal-toggle="cell-attribute-modal" data-modal-target="cell-attribute-modal" data-tooltip-target="tooltip-add-cell-attribute">
        ...
        <div id="tooltip-add-cell-attribute" role="tooltip" className="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip 
        dark:bg-gray-700">
        ...
        <Modal className="bg-sky-600" id="cell-attribute-modal" tabIndex={-1} aria-hidden="true" show={openModal} size="md" popup onClose={onCloseModal}>
            <Modal.Header>
                Tambahkan atribut sel
            </Modal.Header>
