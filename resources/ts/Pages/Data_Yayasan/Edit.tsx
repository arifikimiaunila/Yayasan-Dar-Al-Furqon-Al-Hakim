...
interface DataYayasanFormProps {
  yayasan_id: number;
  initialData?: {
    alamat: string;
    email: string;
...
const DataYayasanForm: React.FC<DataYayasanFormProps> = ({
  yayasan_id,
  initialData,
}) => {
  ...
  return (
    <SuperAdmin>
      <Head title="Edit Data Yayasan" />
      <div className="max-w-3xl mx-auto p-4 sm:p-6 bg-white shadow rounded">
        <h2 className="text-xl sm:text-2xl font-bold mb-4">Edit Data Yayasan</h2>
        ...
        <form
          action={route("data_yayasan.update", yayasan_id)}
          method="post"
          className="space-y-4"
        >
          <div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="w-full sm:w-auto">
              <BlueButton
                href={route("data_yayasan.update", yayasan_id)}
                method="put"
                as="button"
              >
                Update Data Yayasan
            ...
            <div className="w-full sm:w-auto">
              <RedButton href={route("home")}>Cancel</RedButton>
            </div>
          </div>
        </form>
      </div>
